...
from screens.launch.widgets.layouts import (
    TDKLaunchButtonsRegionLayout,
    TDKLaunchLayout,
...
class TDKScreenLaunch(Screen):
    def __init__(self, **kwargs):
        ...
        headerLayout = BoxLayout(
            orientation="vertical", spacing=20, padding=15, size_hint=(1.0, 0.66)
        )
        buttonZoneLayout = TDKLaunchButtonsZoneLayout()
        buttonCreditsLayout = BoxLayout(orientation="horizontal")
        buttonActionsLayout = BoxLayout(orientation="horizontal")
        ...
        buttonCredits = TDKLaunchButton(
            txt="Credits & Thanks",
            bg_color_down=(0.6, 1, 0.6, 1),
        )
        buttonLogin = TDKLaunchButton(
            txt="Author Login",
        )
        buttonOptions = TDKLaunchButton(
            txt="Display Settings",
        )
        # Build and fill the zone which contains the buttons
        add_widgets(
            buttonCreditsLayout,
            [empty_spacer(hori=0.33), buttonCredits, empty_spacer(hori=0.33)],
        )
        ...
        add_widgets(
            buttonActionsLayout,
            [
                empty_spacer(hori=0.05),
                buttonLogin,
                empty_spacer(hori=0.15),
                buttonOptions,
                empty_spacer(hori=0.05),
            ],
        )
        ...
        add_widgets(
            buttonRegionLayout,
            [empty_spacer(hori=0.33), buttonZoneLayout, empty_spacer(hori=0.33)],
        )
        # Use a small empty spacer for the license info layout
        licenseLabel = Label(
            text="unlicensed so far", font_size="14pt", size_hint=(1.0, 0.01)
        )
